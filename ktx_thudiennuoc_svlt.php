...
//                var idbdt=document.getElementById('idbacdaotao').value;
                ...
                var url="ktx_thudiennuoc_svlt.php?idday="+idday+"&idphong="+idphong;
                window.location=url;
            }
    ...
    echo "function tht(){ "; 
      echo "if(confirm('Thông tin chính xác?')){ "; 
          ...
          //var num = $('#dshp input[type=checkbox]:checked').length;           
           ...
           echo "if(str!=\"\"){ "; 
                echo "document.thutiendiennuoclt.submit();";  
                //echo "alert(str);"; 
...
echo "</script>";
        ...
        $kt=0;
        $query_kt="select * from thutiendiennuoclt ";
        $result_kt = mysqli_query($link,$query_kt);  
                            $totalRows_kt=mysqli_num_rows($result_kt); 
                            while ($row_kt = mysqli_fetch_array ($result_kt))
                            {
                                if(($row_kt["ID_PHONG"]==$iphong)&&($row_kt["THANGNOP"])==$thang_nop)
                                {
                                    $kt=$kt+1;
                                    echo "id phòng ".$row_kt["ID_PHONG"];
                                    echo " tháng nộp ".$row_kt["THANGNOP"];
                                }
                            }
...
//                for($i=0; $i<$sopt-1; $i++){
                $query="insert into thutiendiennuoclt(ID_LIENTHONG,ID_PHONG,GIADIEN,CSDIENCU,CSDIENMOI,"
                        . "GIANUOC,CSNUOCCU,CSNUOCMOI,THANHTIEN,NGAYTHU,THANGNOP,NGUOITHU) "
                        . "values($idlienthong,$iphong,$dongiadien,$chisodiencu,$chisodienmoi,$dongianuoc,"
...
//                        echo "</div>";
                        ...
                        echo "<div>"; 
                        echo "<div class=\"form-group\">";
                        echo "<form name=\"thutiendiennuoclt\" action=\"ktx_thudiennuoc_svlt.php\" method=\"post\">";
...
//                        echo "</div>";
                        ...
                        echo "<tr>";
                            $totalRows_csdcu=0;
                            $query_csdcu="select CSDIENMOI from thutiendiennuoclt where ID_PHONG=$idphong"; //echo $query_csdcu;
                            $result_csdcu = mysqli_query($link,$query_csdcu);
                            $totalRows_csdcu=mysqli_num_rows($result_csdcu); 
                            
                                while ($row_csdcu = mysqli_fetch_array ($result_csdcu))     
                                {      
...
//                            }
                        ...
                        echo "<tr>";
                            $totalRows_csncu=0;
                            $chisonuoccu=0;
                            $query_csncu="select CSNUOCMOI from thutiendiennuoclt where ID_PHONG=$idphong"; //echo $query_csdcu;
                            $result_csncu = mysqli_query($link,$query_csncu);
                            $totalRows_csncu=mysqli_num_rows($result_csncu); 
                            
                                while ($row_csncu = mysqli_fetch_array ($result_csncu))     
                                {      
...
//                        echo "<input type=\"hidden\" name=\"chon\" id=\"chon\"> ";                       
                        ...
                        echo "<div>"; 
                        echo "<div class=\"form-group\">";
                        echo "<form role=\"form\" action=\"ktx_danhsachphongdanoptiendiennuoclt.php\" method=\"post\">";
               ...
               <div class="panel">
                    ...
                    <div class="table-responsive">
                        ...
                        <?php
                            ...
                            $totalRows = 0;       
                            $stSQL ="select a.ID_LIENTHONG,b.MASV, b.HODEM,b.TEN,b.NGAYSINH,b.PHAI,c.MALOPLT,d.TENPHONG "
                                    . " from dssvltnoitru a,lienthong b,loplt c, phong d "
                                    . " where a.ID_LIENTHONG =b.ID_LIENTHONG and a.ID_PHONG=d.ID_PHONG and "
                                    . " b.ID_LOP=c.ID_LOPLT and a.ID_PHONG=$idphong";//not in (select ID_SINHVIEN from danhsachnoitru where ID_SINHVIEN is not null)";// and xoaten=0 and datotnghiep=0 and not in ()";  
                            //echo $stSQL;
