...
//    $denngay=$_POST["denngay"];
    ...
    $workSheet->getStyle('F1:H1')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
    $workSheet->setCellValueByColumnAndRow(5, 1, 'CỘNG HÒA XÃ HỘI CHỦ NGHĨA VIỆT NAM');
    $workSheet->mergeCells('F2:H2');
    $workSheet->getStyle('F2:H2')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
    ...
    $workSheet->setCellValueByColumnAndRow(0, 5, 'THỐNG KÊ HSSV NỘP TIỀN ĐIỆN NƯỚC TỪ '.$tungay.' ĐẾN '.$denngay);
...
//    $workSheet->getStyle('C7:D7')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
    ...
    $workSheet->setCellValueByColumnAndRow(3, 7, ' THÀNH TIỀN');
    $workSheet->setCellValueByColumnAndRow(4, 7, ' NGÀY THU');
    $workSheet->setCellValueByColumnAndRow(5, 7, ' THÁNG NỘP');
    $workSheet->setCellValueByColumnAndRow(6, 7, ' NGƯỜI THU');
    $workSheet->setCellValueByColumnAndRow(7, 7, ' NGƯỜI NỘP');
    
    $query ="select a.ID_THUTIENDIENNUOC,a.ID_SINHVIEN,a.THANHTIEN,a.NGAYTHU,a.THANGNOP,a.NGUOITHU,"
            . "b.MASV,b.HODEM,b.TEN,c.TENPHONG "
                . "from dsphieuthutiendiennuoc a,sinhvien b, phong c "
    ...
    $workSheet->getColumnDimension('H')->setWidth(22);
...
//    $workSheet->getColumnDimension('J')->setAutoSize(true);
    while($row=mysql_fetch_object($result)){
        ...
        $workSheet->setCellValueByColumnAndRow($tuCot+1, $tuDong, $row->ID_THUTIENDIENNUOC);
        $workSheet->setCellValueByColumnAndRow($tuCot+2, $tuDong, $row->TENPHONG);
        $workSheet->setCellValueByColumnAndRow($tuCot+3, $tuDong, $row->THANHTIEN);
        $workSheet->setCellValueByColumnAndRow($tuCot+4, $tuDong, $row->NGAYTHU);
        $workSheet->setCellValueByColumnAndRow($tuCot+5, $tuDong, $row->THANGNOP);
        $workSheet->setCellValueByColumnAndRow($tuCot+6, $tuDong, $row->NGUOITHU);
        $workSheet->setCellValueByColumnAndRow($tuCot+7, $tuDong, $row->HODEM.' '.$row->TEN);
    ...
    $stt=$stt-1;
    $workSheet->mergeCells("A".(8+$somautin).":H".(8+$somautin));
...
//    $workSheet->setCellValueByColumnAndRow(1, $tuDong+1, 'Bằng chữ: '.convert_number_to_words($tongsotien));
    ...
    $name="tk_sv_nop_tiendiennuoc_tn_dn.xls";
    //header('Content-Type: application/vnd.ms-excel');
