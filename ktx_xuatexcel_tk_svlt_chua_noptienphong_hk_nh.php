...
//        else echo " ";
    ...
    $workSheet->setCellValueByColumnAndRow(0, 5, 'THỐNG KÊ SVLT CHƯA NỘP TIỀN PHÒNG HỌC KỲ '.$hocky.' NĂM HỌC '.$namhoc);
...
//    $workSheet->setCellValueByColumnAndRow(0, 6, 'Từ ngày '.$tungay.' đến ngày '.$denngay);
    ...
    $query ="select b.ID_LIENTHONG,b.MASV,b.HODEM,b.TEN,b.NGAYSINH,c.TENPHONG,LEFT(c.TENPHONG,1) as DAY,"
            . "e.MALOPLT from lienthong b, phong c, dssvltnoitru d, loplt e "
            . "where d.ID_LIENTHONG=b.ID_LIENTHONG and c.ID_PHONG=d.ID_PHONG and "
            . "e.ID_LOPLT=b.ID_LOP and "
            . " b.ID_LIENTHONG not in (select ID_LIENTHONG from tienphongsinhvienlt where HOCKY='$hocky' "
            . "and NAMHOC='$namhoc' and ID_LIENTHONG is not null)";// and xoaten=0 and datotnghiep=0 and not in ()";  
    //echo $query;
...
//    $workSheet->getColumnDimension('J')->setAutoSize(true);
    while($row=mysqli_fetch_object($result)){
        ...
        $workSheet->setCellValueByColumnAndRow($tuCot+4, $tuDong, $row->MALOPLT);
...
//    $workSheet->setCellValueByColumnAndRow(7, $tuDong, $tongsotien);
    ...
    $now = getdate(); 
    $workSheet->mergeCells("D".(10+$somautin).":H".(10+$somautin));
    $workSheet->getStyle("D".(10+$somautin).":H".(10+$somautin))->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
    ...
    $workSheet->mergeCells("D".(11+$somautin).":H".(11+$somautin));
    $workSheet->getStyle("D".(11+$somautin).":H".(11+$somautin))->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
...
//    $workSheet->setCellValueByColumnAndRow(1, $tuDong+1, 'Bằng chữ: '.convert_number_to_words($tongsotien));
    ...
    $name="tk_svlt_chua_nop_tienphong_hk_nh.xls";
    //header('Content-Type: application/vnd.ms-excel');
