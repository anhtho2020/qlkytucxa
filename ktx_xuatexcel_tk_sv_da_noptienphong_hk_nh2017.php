...
//        else echo " ";
    ...
    $workSheet->setCellValueByColumnAndRow(0, 5, 'THỐNG KÊ HSSV NỘP TIỀN PHÒNG HỌC KỲ '.$hocky.' NĂM HỌC '.$namhoc);
...
//    $workSheet->setCellValueByColumnAndRow(0, 6, 'Từ ngày '.$tungay.' đến ngày '.$denngay);
    ...
    $query ="select a.ID_SINHVIEN,a.THANHTIEN,a.NGAYTHU,a.HOCKY,a.NAMHOC,b.MAHSSV,b.HODEM,b.TEN,b.NGAYSINH,"
                ...
                . "where a.ID_SINHVIEN=b.ID_DSHSSV and d.ID_SINHVIEN=a.ID_SINHVIEN and "
            . "c.ID_PHONG=d.ID_PHONG and  "
            . " a.HOCKY='$hocky' and NAMHOC='$namhoc'";//echo $query;
//    echo $query;
...
//    $workSheet->getColumnDimension('J')->setAutoSize(true);
    ...
    $workSheet->mergeCells("D".(9+$somautin).":G".(9+$somautin));
    $workSheet->getStyle("D".(9+$somautin).":G".(9+$somautin))->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
    $workSheet->setCellValueByColumnAndRow(3, $tuDong+1, convert_number_to_words($tongsotien));
    //$workSheet->setCellValueByColumnAndRow(6, $tuDong, $tongsotien);
    ...
    $workSheet->setCellValueByColumnAndRow(0, $tuDong+2, 'Danh sách gồm: '.$stt.' học sinh sinh viên.');
    
    //$workSheet->mergeCells("B".(11+$somautin).":C".(11+$somautin));
    $workSheet->setCellValueByColumnAndRow(1, $tuDong+3, 'Phòng công tác CT-HSSV');
    ...
    $workSheet->setCellValueByColumnAndRow(3, $tuDong+3, 'Cần thơ, ngày '. $now["mday"] . ' tháng '. $now["mon"] . ' năm ' . $now["year"] );
    $workSheet->getStyle("D".($tuDong+2))->getFont()->setItalic(true);
    
    $workSheet->mergeCells("D".(12+$somautin).":G".(12+$somautin));
    $workSheet->getStyle("D".(12+$somautin).":G".(12+$somautin))->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
    $workSheet->setCellValueByColumnAndRow(3, $tuDong+4, 'Người lập bảng' );
...
//    $workSheet->setCellValueByColumnAndRow(1, $tuDong+1, 'Bằng chữ: '.convert_number_to_words($tongsotien));
    ...
    $name="tk_sv_nop_tienphong_hk_nh.xls";
    //header('Content-Type: application/vnd.ms-excel');
