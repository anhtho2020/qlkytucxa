...
//    mysql_query("SET CHARACTER SET utf8",$connect);
    ...
    $workSheet->setCellValueByColumnAndRow(0, 5, 'DANH SÁCH PHIẾU THU TIỀN PHÒNG KHÁCH ');
...
//    $workSheet->mergeCells('C7:D7');
    ...
    $workSheet->setCellValueByColumnAndRow(4, 7, 'NGÀY MƯỚN');
    $workSheet->setCellValueByColumnAndRow(5, 7, ' SỐ NGÀY');
    $workSheet->setCellValueByColumnAndRow(6, 7, ' ĐƠN GIÁ');
    $workSheet->setCellValueByColumnAndRow(7, 7, ' THÀNH TIỀN ');
    $workSheet->setCellValueByColumnAndRow(8, 7, ' NGÀY THU ');
    $workSheet->setCellValueByColumnAndRow(9, 7, ' NGÀY IN PHIẾU');
    $workSheet->setCellValueByColumnAndRow(10, 7, ' NGƯỜI THU ');
    $workSheet->setCellValueByColumnAndRow(11, 7, ' DIỄN GIẢI ');
//    $workSheet->setCellValueByColumnAndRow(12, 7, 'THÁNG NỘP');
//    $workSheet->setCellValueByColumnAndRow(13, 7, 'NGƯỜI THU');
//   
    
    $query ="select a.ID_THUTIENPHONGKHACH,a.ID_KHACH,a.ID_PHONG,a.NGAYMUON,a.SONGAY,a.DONGIA,"
...
//    $workSheet->getColumnDimension('N')->setWidth(15);
    while($row=mysqli_fetch_object($result)){
        ...
        $workSheet->setCellValueByColumnAndRow($tuCot+4, $tuDong, $row->NGAYMUON);
        ...
        $workSheet->setCellValueByColumnAndRow($tuCot+7, $tuDong, $row->THANHTIEN);
        $workSheet->setCellValueByColumnAndRow($tuCot+8, $tuDong, $row->NGAYTHU);
        $workSheet->setCellValueByColumnAndRow($tuCot+9, $tuDong, $row->NGAYINPHIEU);
        $workSheet->setCellValueByColumnAndRow($tuCot+10, $tuDong, $row->NGUOITHU);
...
//    echo "STT la".$tt;
    ...
    $workSheet->mergeCells("C".(9+$somautin).":L".(9+$somautin));
//    $workSheet->getStyle("C".(9+$somautin).":L".(9+$somautin))->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
    ...
    $now = getdate(); 
    $workSheet->mergeCells("G".(10+$somautin).":L".(10+$somautin));
    $workSheet->getStyle("G".(10+$somautin).":L".(10+$somautin))->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
    ...
    $name="dsphieuthutienphongkhachKT.xls";
    //header('Content-Type: application/vnd.ms-excel');
